...
- Status: accepted 2026-08-21; two consequences superseded 2026-09-01 — see Consequences
- Phase: 6
...
- Migrations are pasted by the owner before promotion, never by CI. **Superseded 2026-09-01 (story #114):** still never by CI, and still before promotion — but no longer a paste and no longer the owner's keystrokes. A session applies the file with `npm run migrate:live <file>` at the owner's go-ahead, using the Supabase personal access token from `.env.local`; the command verifies the payload the database actually received before applying it. **Deciding** to apply, and the `develop` → `release` promotion, remain the owner's — which is the half this consequence was protecting. *(Note added 2026-09-17 at a groom-backlog sweep.)*
...
**Re-read 2026-09-17.** #114 moved the apply into a session command, which removes most of the friction this condition was watching for — the risk it names is now *promoting before the session applies the file*, not *before the owner pastes it*. The remedy it proposes is also partly overtaken: the authority already sits on this machine in `.env.local`, so the open question is whether it should also sit in CI, which is a strictly larger grant. Left standing rather than rewritten, because the failure it counts — a promotion that outruns its migration — is unchanged and still worth counting.
